...
class LegendWindow(ctk.CTkToplevel):
    ...
    def _setup_ui(self):
        ...
        ctk.CTkButton(self, text="Cerrar Ayuda", fg_color="#3498db", command=self.destroy).pack(pady=20)
...
class MatrixWindow(ctk.CTkToplevel):
    ...
    def _setup_ui(self):
        ...
        # --- GENERACIÓN DE LA MATRIZ 8x8 CON BLOQUEO ACTUALIZADO ---
        for r in range(8):
            for c in range(8):
                key = (r, c)
                
                # CONDICIÓN DE BLOQUEO ACTUALIZADA: 
                # Si Columna >= Fila (Diagonal Principal + Triángulo Superior)
                if c >= r:
                    # Celda Bloqueada (Oscura)
                    btn = ctk.CTkButton(grid_frame, text="", width=60, height=45, 
                                        fg_color="#444444",      # Gris Oscuro
                                        state="disabled",        # Deshabilitado
                                        border_width=0)          # Sin borde
                else:
                    # Celda Activa (Solo Triángulo Inferior)
